...
assert.doesNotMatch(main, /extensions:\s*\['md', 'markdown', 'txt'\]/);
assert.doesNotMatch(main, /\{\s*name:\s*'Text',\s*extensions:\s*\['txt'\]\s*\}/);
...
assert.match(main, /ipcMain\.handle\('native:create-project-file'/);
assert.match(main, /ipcMain\.handle\('native:create-project-folder'/);
assert.match(main, /resolveProjectPath/);
assert.match(main, /flag:\s*'wx'/);
assert.match(main, /同名文档已存在/);
...
assert.match(main, /function isSupportedMarkdownPath\(filePath\)/);
assert.match(main, /if \(!isSupportedMarkdownPath\(filePath\)\) return null/);
...
assert.match(preload, /createProjectFile/);
assert.match(preload, /createProjectFolder/);
...
assert.equal(packageJson.scripts['dmg:mac'], 'pnpm run package:mac && node scripts/dmg-mac-package.cjs');
assert.equal(packageJson.scripts['zip:mac'], 'pnpm run package:mac && node scripts/zip-mac-package.cjs');
assert.equal(packageJson.scripts['zip:win'], 'pnpm run package:win && node scripts/zip-win-package.cjs');
...
assert.match(zipMac, /Run pnpm run package:mac first/);
...
assert.match(zipWin, /Run pnpm run package:win first/);
