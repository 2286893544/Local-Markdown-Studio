...
function renderProjectFiles() {
  if (!state.projectFiles.length) {
    ...
    elements.projectFiles.innerHTML = '<p class="empty-outline">点击“打开项目”读取文件夹中的 Markdown</p>';
    return;
  }
...
function renderOutline() {
  ...
  elements.outline.innerHTML = headings
    .map(
      (heading) =>
        `<button class="outline-item level-${heading.level}" type="button" data-target="${heading.id}">${escapeHtml(heading.text)}</button>`,
    )
    .join('');

  elements.outline.querySelectorAll('[data-target]').forEach((button) => {
