...
- 最近打开文件 / 最近打开项目入口
- 当前文件保存、另存为和未保存变更提醒
- 粘贴或拖入项目外图片时，原生桌面模式可自动复制到当前文档旁的 `assets/` 目录；拖入项目内已有图片时会按当前文档位置插入真实相对路径
- 图片资产面板，展示项目图片的引用关系、引用次数和未引用状态
- 项目文档快速打开，支持按文件名或路径筛选
- 项目内可直接新建 Markdown 文档和文件夹
- 项目内全文搜索，支持从结果打开文档并跳到匹配项
- 项目健康检查，集中提示缺失图片、未引用图片资产、绝对路径图片、重复标题和失效文档链接，并支持定位问题文档
- 绝对路径图片一键修复为相对路径，降低交付文档泄露本机路径的风险
- 当前文档反向链接和失效链接检查
- 文档目录自动提取，支持层级折叠
...
当前应用专注于本地 Markdown 阅读、编辑、组织和导出。默认项目扫描只读取 `.md` 文件，可在应用内通过“扫描设置”增加 `.markdown`；非 Markdown 文档不会作为文档打开或纳入项目扫描。项目健康检查会同时扫描常见图片资产，用于发现文档图片引用和本地素材管理问题。
